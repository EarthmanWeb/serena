...
class MemoryManager:
    ...
    def __init__(
        ...
        :param project_root: the absolute path to the project root (used for resolving relative extra memory paths)
    ...
    def load_memory(self, name: str) -> str:
        ...
        # Prefer an existing file found in the primary or an extra dir; fall back to the
        # sandbox-validated primary path (which raises for out-of-sandbox names).
    ...
    def save_memory(self, name: str, content: str, is_tool_context: bool) -> str:
        ...
        # Update an existing memory in place (including one that lives in an extra dir),
        # otherwise create it under the primary dir via the sandbox-validated path.
        memory_file_path = self._find_existing_memory(name) or self.get_memory_file_path(name)
    ...
    def _list_extra_memories(self, topic: str = "") -> MemoriesList:
        """List memories from extra memory directories, skipping any already known from the primary dir."""
        result = self.MemoriesList()
        seen: set[str] = set()
        # Collect names already known from the primary dir to avoid duplicates
        if self._project_memory_dir is not None:
    ...
    def move_memory(self, old_name: str, new_name: str, is_tool_context: bool) -> str:
        ...
        old_path = self._find_existing_memory(old_name) or self.get_memory_file_path(old_name)
