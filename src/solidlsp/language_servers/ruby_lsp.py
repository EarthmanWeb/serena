...
class RubyLsp(SolidLanguageServer):
    ...
    def _setup_runtime_dependencies(
        ...
        try:
            version_args = ["-v", ruby_lsp_version] if ruby_lsp_version else []
            install_cmd = gem_install_cmd + version_args
            subprocess.run(install_cmd, check=True, capture_output=True, cwd=repository_root_path)
